...
export default function HomeClient() {
  ...
  const loadListings = async () => {
    try {
      ...
      let nextListings: any[] = []
      if (Array.isArray(response.data)) {
        nextListings = response.data
      } else if (response.data?.data && Array.isArray(response.data.data)) {
        nextListings = response.data.data
      }
              ...
              {[
                {
                  ...
                  desc: 'Укажите город, даты заезда и выезда, количество гостей',
                },
                {
                  icon: Calendar,
                  title: 'Сравните варианты',
                  desc: 'Изучите предложения, сравните цены, удобства и расположение',
                },
                {
                  icon: CheckCircle2,
                  title: 'Бронируйте напрямую',
                  desc: 'Свяжитесь с владельцем и забронируйте без посредников',
                },
              ].map((step, i) => (
            ...
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-600">
                {listings.length > 0
                  ...
                  : 'Используйте поиск выше, чтобы найти жильё'}
              </p>
            </div>
            {listings.length > 0 && (
              <div className="flex gap-2 bg-gray-100 rounded-lg p-1">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-md transition-all font-medium cursor-pointer ${
                    viewMode === 'map' ? 'bg-white text-primary shadow-sm' : 'text-gray-600 hover:text-gray-900'
                  }`}
                >
                  Карта
                </button>
